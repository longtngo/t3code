...
These rules override the ones above for this fork only.
...
- `pnpm test` runs each package's own `test` script **from that package's directory**. For the web
  app that is `vp test run --passWithNoTests --project unit --project dom`. Run it with both, or
...
- **Run web tests from `apps/web`, not from the repo root.** A root-level path filter
  ...
  file passes 9/9 from `apps/web`. Named projects only resolve there too: at the root even
  `--project dom`, which does exist in `apps/web`, errors with
  `No projects matched the filter "dom"`. (Verified 2026-09-07 with `dom` rather than a name that
  exists nowhere, so it distinguishes "the root cannot resolve named projects" from "that project
  is not defined".)
- **`apps/web/vite.config.ts` registers two test projects, `unit` and `dom`** — the `dom` one runs
  `src/**/*.dom.test.tsx` under happy-dom with a 120s hook and test timeout.
- There is **no `browser` test project and no browser test file**, and no `*.browser.tsx` source
  file exists — the nine `__screenshots__/*.browser.tsx` entries are PNG **directories** left by
  tests that were deleted, and none of them is tracked in git. Nothing is going uncovered.
