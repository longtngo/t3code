import { isWindowsAbsolutePath, isWindowsDrivePath } from "@t3tools/shared/path";
...
/**
 * Whether an absolute path sits under a root a real filesystem uses. Shared
 * with the chat prose linkifier so the link gate and the prose gate cannot
 * disagree about what a path is. (FORK-ONLY export.)
 */
export function isKnownFilesystemRootPath(path: string): boolean {
  // Drive paths only: an escaped backslash in prose is not a UNC path.
  if (isWindowsDrivePath(path)) return true;
