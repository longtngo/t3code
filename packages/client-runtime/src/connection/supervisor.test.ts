...
describe("EnvironmentSupervisor", () => {
  ...
  // This fork caps the reconnect backoff at 3s (see RETRY_DELAYS_MS); upstream
  // uses a 1s base and a 16s cap.
  ...
  it.effect("explicit retry starts a fresh backoff sequence", () =>
    Effect.gen(function* () {
      ...
      // A reset sequence restarts at RETRY_DELAYS_MS[0], which is 500ms in this
      // fork (upstream's first delay is 1s).
