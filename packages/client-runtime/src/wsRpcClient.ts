...
export interface WsRpcClient {
  ...
  readonly pushSubscriptions: {
    readonly register: RpcUnaryMethod<typeof WS_METHODS.pushSubscriptionsRegister>;
  };
  readonly cloud: {
...
export function createWsRpcClient(
  ...
  return {
    ...
    attachments: {
      upload: (input) =>
        transport.request((client) => client[WS_METHODS.attachmentsUpload](input)),
    },
    filesystem: {
    ...
    llmModels: {
      subscribe: (listener, options) =>
        transport.subscribe(
          (client) => client[WS_METHODS.subscribeLlmModels]({}),
          listener,
    ...
    pushSubscriptions: {
      register: (input) =>
        transport.request((client) => client[WS_METHODS.pushSubscriptionsRegister](input)),
    },
    cloud: {
