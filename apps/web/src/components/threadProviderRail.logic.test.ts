...
describe("resolveThreadProviderInstanceId", () => {
  ...
  it("prefers a live session's provider over the stored selection", () => {
    expect(
      resolveThreadProviderInstanceId(thread({ selection: uniSub, sessionInstance: personalSub })),
    ).toBe(personalSub);
  });

  it("falls back to the stored selection when a session carries no provider", () => {
    expect(resolveThreadProviderInstanceId(thread({ selection: uniSub, hasSession: true }))).toBe(
      uniSub,
    );
  });
});
...
describe("resolveThreadProviderPresentation", () => {
  ...
  it("returns nothing when the instance has no accent colour", () => {
    expect(
      resolveThreadProviderPresentation(uniSub, [
        { instanceId: uniSub, driver: claudeKind, displayName: "UniSub" },
      ]),
    ).toBeUndefined();
  });
  ...
  it("treats a malformed accent as absent rather than passing it to the DOM", () => {
    for (const accentColor of ["red", "#fff", "#12345g", "ea580c", ""]) {
      expect(
        resolveThreadProviderPresentation(uniSub, [
          { instanceId: uniSub, driver: claudeKind, accentColor },
        ]),
      ).toBeUndefined();
    }
  });
  ...
  it("announces the instance id when the snapshot carries no display name", () => {
    expect(
      resolveThreadProviderPresentation(uniSub, [
        { instanceId: uniSub, driver: claudeKind, accentColor: "#ea580c" },
      ]),
  ...
  it("keeps instances that share an accent distinguishable by name", () => {
    const shared: ReadonlyArray<ThreadProviderAccentSource> = [
      {
        instanceId: ProviderInstanceId.make("claudeAgent_qwen"),
        driver: claudeKind,
        displayName: "Qwen",
        accentColor: "#7c3aed",
      },
    ...
    const qwen = resolveThreadProviderPresentation(
      ProviderInstanceId.make("claudeAgent_qwen"),
      shared,
    );
