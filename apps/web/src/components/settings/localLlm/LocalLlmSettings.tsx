...
/**
 * Edit a provider/model's grouped launch-arg tokens: one flag group per line (e.g.
 * `--reasoning-budget 0`). The server splits each line into flat spawn tokens. Committed
 * on blur so a keystroke doesn't write settings on every character.
 */
function GroupedArgsInput(props: {
  ...
  return (
    <textarea
      ...
      onBlur={commit}
      placeholder={props.placeholder ?? "one flag group per line, e.g. --reasoning-budget 0"}
      spellCheck={false}
    />
  );
}
...
export function LocalLlmSettingsPanel() {
  const lm = usePrimarySettings((s) => s.localLlm);
