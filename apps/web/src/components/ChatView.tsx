...
// Fork: the revert prompt-restore callback is required, so the held paint gets a
// noop rather than the live handler — same reasoning as `noopHeldRevert`.
const noopHeldArmRevertPromptRestore = (
...
export default function ChatView(props: ChatViewProps) {
  ...
  // Fork: upstream's revert carries only a turn count, so the message's own
  // text is stashed here and replayed once the message actually leaves the
  // thread. Arming is separate from reverting on purpose - the restore is
  // driven by observing the thread, which is what makes it idempotent.
  const onArmRevertPromptRestore = useCallback(
    (
      messageId: MessageId,
      promptText: string,
  ...
  const rightPanelContent = activeThreadRef ? (
    renderedRightPanelSurface?.kind === "preview" ? (
    ...
    ) : renderedRightPanelSurface?.kind === "trustedFile" ? (
      <Suspense fallback={null}>
        <TrustedFileView
          environmentId={activeProject?.environmentId ?? null}
          absolutePath={renderedRightPanelSurface.absolutePath}
      ...
      <Suspense fallback={null}>
        <FilePreviewPanel
          ...
          fileRepoCwd={
            renderedRightPanelSurface.kind === "file"
              ? renderedRightPanelSurface.repoCwd
              : undefined
          }
            ...
            <div className="relative flex min-h-0 flex-1 flex-col bg-background">
              ...
              <MessagesTimeline
                ...
                onArmRevertPromptRestore={
                  paintOnlyDisplayedTimeline
                    ? noopHeldArmRevertPromptRestore
                    : onArmRevertPromptRestore
                }
                          ...
                          <div className="pointer-events-auto" id={COMPOSER_CONTEXT_STRIP_ID}>
                            {mountComposerContextStrip && (
                              <BranchToolbar
                                ...
                                contextStripVisible={renderComposerContextStrip}
                              />
                            )}
                          </div>
                        </div>
