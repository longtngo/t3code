...
/**
 * The shortcuts strip + gear that live above the composer input. Chips insert their prompt
 * text into the composer (via `onInsert`); the gear opens the manager. The strip stretches so
 * the gear sits at the far right of the row.
 */
export function ComposerShortcutsControls({
  ...
  const shortcuts = useClientSettings((s) => s.composerShortcuts);
  ...
  return (
    <>
      ...
      <ShortcutManagerDialog
        ...
        shortcuts={shortcuts}
        onSave={(next) => {
          updateClientSettings({ composerShortcuts: next });
