...
describe("deriveMessagesTimelineRows", () => {
  it("leads the worktree setup card with the working header", () => {
    ...
    // FORK: upstream's `runningWithQueue` case is absent - it drives the rejected client-side
    // queue (docs/fork/README.md invariant 50).
...
// Ported from ChatView.logic.test.ts, which used to own this helper. Upstream relocated the
// code here and its version scans on past a turn boundary; these two cases are what stops it.
describe("revert target scanning", () => {
  const now = "2026-01-01T00:00:00.000Z";
  const entry = (id: MessageId, role: "user" | "assistant", turnId: TurnId | null) => ({
    id,
    kind: "message" as const,
    createdAt: now,
    message: { id, role, text: id, turnId, createdAt: now, updatedAt: now, streaming: false },
  });
  const summary = (assistantMessageId: MessageId, turnId: TurnId, checkpointTurnCount: number) => ({
    turnId,
    checkpointTurnCount,
  ...
  const rowsFor = (
    timelineEntries: ReturnType<typeof entry>[],
    turnDiffSummaries: ReturnType<typeof summary>[],
  ) =>
    deriveMessagesTimelineRows({
      timelineEntries,
      latestTurn: null,
  ...
  const revertTargetOf = (rows: ReturnType<typeof rowsFor>, id: MessageId) => {
    const row = rows.find(
      (candidate) => candidate.kind === "message" && candidate.message.id === id,
    );
    return row?.kind === "message" ? row.revertTurnCount : undefined;
  };
  ...
  it("offers no target when the clicked message's own turn was never checkpointed", () => {
    ...
    const rows = rowsFor(
      [
        entry(clicked, "user", null),
        entry(MessageId.make("orphan-assistant-a"), "assistant", TurnId.make("turn-a")),
        entry(laterAssistant, "assistant", TurnId.make("turn-b")),
      ],
      [summary(laterAssistant, TurnId.make("turn-b"), 4)],
    );

    expect(revertTargetOf(rows, clicked)).toBeUndefined();
  });
  ...
  it("still uses a later assistant message from the same turn", () => {
    ...
    const rows = rowsFor(
      [
        entry(clicked, "user", null),
        entry(MessageId.make("same-assistant-1"), "assistant", turn),
        entry(secondAssistant, "assistant", turn),
      ],
      [summary(secondAssistant, turn, 3)],
    );

    expect(revertTargetOf(rows, clicked)).toBe(2);
  });
});
