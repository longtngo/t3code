...
// Upstream's `stopRowToggle` was declared here. Not carried: its only use was
// the inline expanded-body block on a work-entry row, and this fork opens that
// detail in a modal (`onOpenDetail`) instead, so nothing here needs to stop the
// row's toggle from firing. Carrying it would just be an unused symbol.
...
const PlainWorkEntryRow = memo(function PlainWorkEntryRow(props: {
  ...
  const rowText = displayLabel ?? workEntryDisplayLabel(workEntry, workspaceRoot);
