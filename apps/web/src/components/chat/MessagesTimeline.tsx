  ...
  return (
    <div className="border-b border-border/60 pb-2 pt-1">
      <div className="flex h-6 min-w-0 items-baseline gap-2 px-1 text-sm leading-relaxed text-muted-foreground tabular-nums">
        ...
        {/* Fork: the current step rides beside the timer. Suppressed during
            worktree setup, where upstream's shimmer is the whole message. */}
        {workingStepLabel && !isPreparingWorktree ? (
...
const WorkGroupSection = memo(function WorkGroupSection({
  ...
  const { workspaceRoot, threadRef, routeThreadKey, onImageExpand } = use(TimelineRowCtx);
...
function ExpandedWorkGroupEntries({
  ...
  const { workGroupViewState: viewState, threadRef, onImageExpand } = use(TimelineRowCtx);
  // The expanded group opens work-entry detail in the same modal the collapsed group
  // does, one dialog for the whole list rather than one per row.
  const [detailEntry, setDetailEntry] = useState<TimelineWorkEntry | null>(null);
...
function LiveWorkEntryTimelineRow({ row }: { row: Extract<TimelineRow, { kind: "work-live" }> }) {
  ...
  if (row.entry.agentSpawn) {
    return <AgentSpawnCtaRow workEntry={row.entry} />;
  }
...
// Upstream #4083 also added `workEntryRawCommand` / `buildToolCallExpandedBody`
// here to build an inline expanded body for a work-entry row. Not carried:
// this fork opens that detail in a modal instead, so both helpers would be
// unused. (`stopRowToggle` used to be absent for the same reason and is now
// back for the question-answer body - see its own note.) The class below is kept
...
// `workEntryPreview` / `capitalizePhrase` / `toolWorkEntryHeading` used to build
// a row's label here. Upstream #9093 replaced all three with
// `workEntryDisplayLabel` in MessagesTimeline.logic.ts, which the live row
// already shared; keeping local copies would be two spellings of one label.

// Carried, narrowly. This used to be absent: its only use was the inline
// expanded-body block on a work-entry row, which this fork rejects in favour of
// a modal (`onOpenDetail`). Upstream's question-answer history (#9871) is a
// separate feature the fork does keep, and it renders interactive content inside
// a row whose click opens that modal - so the answer body still has to stop the
...
const SimpleWorkEntryRow = memo(function SimpleWorkEntryRow(props: {
  ...
  const { workEntry, count, workspaceRoot, isExpandedToolGroupEntry, onOpenDetail, displayLabel } =
    props;
...
// Upstream split the old SimpleWorkEntryRow body out to here so spawn CTA rows
// could short-circuit before any hooks. The fork's work-log detail handler has
// to travel with the body it belongs to, so `onOpenDetail` is threaded through
...
const PlainWorkEntryRow = memo(function PlainWorkEntryRow(props: {
  ...
  // `displayLabel` is the group's precomputed label (upstream #9093); the
  // helper is the same text this file used to build inline, now shared with
  // the live row in MessagesTimeline.logic.ts.
  // A submitted question answer is its own row (upstream #9871); its payload is rendered
  // below rather than folded into the label.
  const rowText = workEntry.questionAnswer
    ? "Question answer submitted"
    : (displayLabel ?? workEntryDisplayLabel(workEntry, workspaceRoot));
  // The count rides on the row text, so it reaches the accessible name too: a
  // row that stands for five occurrences must not announce as one.
  const displayText = count > 1 ? `${rowText} \u00d7${count}` : rowText;
  ...
  //
  // Upstream #8936 rendered a viewed image inline on the row. The fork keeps ONE
  // surface for work-entry detail, so the image moved into the dialog and this
  // gate only asks whether a path exists — resolving the asset is the dialog's job.
  const canOpenDetail =
    hasWorkEntryDetail(workEntry) || workEntryViewedImagePath(workEntry) !== null;
        ...
        <div className="flex min-w-0 flex-1 items-center gap-1.5">
          ...
          <span
            className={cn(
              ...
              !canOpenDetail && "invisible",
            )}
            aria-hidden
          >
            {/* Points at the detail modal, not at a body below, so it does not rotate. */}
            <ChevronRightIcon className="size-3 shrink-0 text-icon-muted opacity-70" />
          </span>
        </div>
      </div>
      {/* Upstream's inline viewed-image and expanded-body blocks are rejected here: this
          fork opens work-entry detail in a modal instead (docs/fork/README.md invariant 4).
          The question-answer history is a different feature and stays. */}
      {workEntry.questionAnswer ? (
