...
import {
  ...
  // Minimize2Icon is deliberately absent: the fork's expand control opens the
  // side panel instead of toggling in place, so there is no collapse glyph.
  OctagonAlertIcon,
  PresentationIcon,
  SparklesIcon,
  TriangleAlertIcon,
...
import { Dialog, DialogPanel, DialogPopup, DialogTitle } from "./ui/dialog";
...
interface ChatMarkdownProps {
   ...
   * renders it without a thread — the file viewer and the pull request panel.
   *
   * Was the fork's `fileEnvironmentId`, which fed "Open in new tab" only, leaving
   * the other chip actions resolving through the PRIMARY environment. Upstream's
   * prop closes that: it also drives remote-open resolution, the editor hook,
   * server config and the workspace basename lookup.
   */
  ...
  return (
    <div ref={containerRef} className="chat-markdown-table-container">
      ...
      <Dialog open={expandedOpen} onOpenChange={setExpandedOpen}>
        <DialogPopup className="max-h-[88vh] max-w-[min(96vw,72rem)]">
 ...
 */
/**
 * Shiki highlighting runs in a Suspense boundary and can throw (an unknown
 * language, a highlighter that failed to load). Without this, one bad code
 * block blanks the whole message; with it, that block falls back to plain text.
 */
    ...
    <span className="inline-flex max-w-full items-center align-middle">
      <Tooltip>
        ...
        >
          ...
          {/* Inline utilities, not a named class: the fork briefly used a
              `markdown-file-link-tooltip-scroll` class that was never defined in any
              stylesheet, so the thin scrollbar silently fell back to the browser default. */}
...
function useChatMarkdownState({
  ...
  const markdownFileLinkMetaByHref = useMemo(() => {
    ...
    const register = (href: string) => {
      if (parseComposerContextHref(href)) return;
  ...
  const rehypePlugins = useMemo(
    () => [
      // rehypeRaw + sanitize are the raw-HTML pair; upstream (#4133) lets a
      // caller opt out so XML-like tags in a user message survive as text.
