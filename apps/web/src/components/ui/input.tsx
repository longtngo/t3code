...
/**
 * The bordered, focus-ringed shell every text control in the app sits inside, and the
 * default-size classes for the control itself. Exported so a field that is not an
 * `<input>` - a native `<select>`, say - can be the same physical control as the text
 * fields beside it rather than a lookalike that ends up a couple of pixels short.
 */
const inputControlShellClassName =
...
function Input({
  ...
  return (
    <span
      className={
        cn(
          !unstyled && inputControlShellClassName,
          !unstyled &&
            size === "compact" &&
...
export { Input, inputControlClassName, inputControlShellClassName, type InputProps };
