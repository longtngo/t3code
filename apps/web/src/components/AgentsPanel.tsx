...
// Stable empty default so an omitted prop does not remount the section.
...
function BackgroundProcessesSection({
  ...
  items: ReadonlyArray<BackgroundSidebarItem>;
  onOpen: ((terminalId: string) => void) | undefined;
}) {
  const collapsed = useSidebarViewStore((state) => state.collapsedSections.background);
    ...
    >
      {items.map((item) => (
        <SidebarItemRow
          ...
          detail={item.cwd}
          selected={false}
          // Open the real terminal surface rather than a static buffer dump:
          // the right panel already hosts a live xterm for it.
          onSelect={() => onOpen?.(item.id)}
          onRemove={isTerminalSidebarStatus(item.status) ? () => dismissItem(item.id) : undefined}
        />
      ))}
    </SidebarSection>
  );
...
export function AgentsPanel({
  ...
  environmentId?: EnvironmentId | null;
  threadId?: ThreadId | null;
  /** Background-process (terminal) items for this thread, filtered + sorted. */
  ...
  // The empty state is only empty when there is no live work of EITHER kind:
  // a thread with a running background process has something to show here.
  if (!model.hasAgents && backgroundItems.length === 0) {
    return (
