...
/**
 * Audio playback for a workspace file. Video goes through `WorkspaceVideoPreview`
 * below, which upstream gave a retry and an actions menu; audio has no upstream
 * branch at all, so it keeps this one.
 */
function WorkspaceAudioPreview(props: {
  ...
  const revisionSuffix =
    props.workspaceMutationId === null
      ? ""
      : `${assetUrl._tag === "Success" && assetUrl.url.includes("?") ? "&" : "?"}workspace-revision=${encodeURIComponent(props.workspaceMutationId)}`;
  const audioUrl = assetUrl._tag === "Success" ? `${assetUrl.url}${revisionSuffix}` : null;

  if (assetUrl._tag === "Failure" || (audioUrl !== null && failedUrl === audioUrl)) {
    return (
      <div className="flex min-h-0 flex-1 items-center justify-center px-6 text-center text-xs leading-relaxed text-destructive">
        Unable to play workspace audio.
      </div>
    );
  }

  return assetUrl._tag === "Success" && audioUrl !== null ? (
    <div className="flex min-h-0 flex-1 items-center justify-center overflow-auto p-4">
      {/* No autoPlay: this is a file browser, and a path that starts blaring audio
          is a defect. `preload="metadata"` fetches the header and nothing more. */}
      <audio
        className="w-full max-w-lg"
        src={audioUrl}
        controls
        preload="metadata"
        onError={() => setFailedUrl(audioUrl)}
      />
    </div>
  ) : (
    <div className="flex min-h-0 flex-1 items-center justify-center text-muted-foreground">
