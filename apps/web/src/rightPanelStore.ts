...
interface RightPanelStoreState {
  ...
  open: (
    ref: ScopedThreadRef,
    kind: Exclude<RightPanelKind, "file" | "trustedFile" | "terminal" | "pull-request">,
  ) => void;
...
const singletonSurface = (
  kind: Exclude<RightPanelKind, "file" | "trustedFile" | "preview" | "terminal" | "pull-request">,
): RightPanelSurface => {
  switch (kind) {
    case "diff":
      return { id: "diff", kind };
...
export const useRightPanelStore = create<RightPanelStoreState>()(
  persist(
    (set, get) => ({
      ...
      openFile: (ref, requestedPath, line, repoCwd) =>
        set((state) =>
          userAction(state, scopedThreadKey(ref), (current) => {
