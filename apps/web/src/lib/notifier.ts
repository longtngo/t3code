/**
 * Thread-completion OS notifications (web + Electron desktop).
 *
 * Raised when a thread's turn transitions running -> terminal while the user is
 * not actively viewing that thread. On the browser this uses the W3C
 * `Notification` API; inside the Electron shell it routes through the
 * `desktopBridge.showNotification` IPC so the main process owns presentation and
 * click handling. Mobile has its own local-notification path (see the mobile
 * agent-awareness feature) and does not use this module.
 *
...
function dispatchNotification(ref: ScopedThreadRef, completion: ThreadCompletion): void {
  ...
  if (isElectron) {
    ...
    // predates showNotification degrades to a no-op instead of throwing.
 ...
 * settings toggle). Electron handles presentation in the main process, so no
 * renderer permission is required there.
 */
