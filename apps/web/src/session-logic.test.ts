...
describe("deriveWorkLogEntries quiet-timeline guarantee", () => {
  it("concurrent subagents replace their launch tools with one lifecycle row", () => {
    // FORK: `activitiesInOrder` memoizes the sorted list by ARRAY IDENTITY, so a
    // caller that pushes into the same array and re-derives reads the first
    // snapshot forever (the memo's own contract). Upstream's version of this test
    // mutates one array in place; each derive here gets a fresh copy instead.
    const activities: OrchestrationThreadActivity[] = [];
...
describe("session activity performance", () => {
  ...
  it("reuses every derived row when one activity is appended", () => {
    ...
    // Reuse alone would also hold if the appended activity derived nothing, so the
    // new row is checked too (adopted from upstream #8585).
