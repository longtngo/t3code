...
import { deriveLatestContextWindowSnapshot } from "../../../packages/client-runtime/src/context/contextWindow.ts";
...
describe("projectActivityPayload", () => {
  ...
  function withoutDetailPayload<T extends { detailPayload?: unknown }>(
    entries: ReadonlyArray<T>,
  ): Array<Omit<T, "detailPayload">> {
    return entries.map(({ detailPayload: _detailPayload, ...rest }) => rest);
  }
  ...
  it("keeps current web and mobile derived output identical for every tool item type", () => {
    for (const activity of fixtures) {
      ...
      // Two carve-outs, from both sides of the fork, and both are still load-bearing.
      if (activity === fixtures[4]) {
        // Upstream (#5482): MCP is the one deliberate difference — the expanded
        // row's toolData loses result bulk but keeps the rendered identity fields.
      ...
      // Fork: `detailPayload` is excluded deliberately. This fork's work log retains
      // the raw tool payload verbatim (session-logic sets `entry.detailPayload =
      // payload`) so the entry detail view can render it as inspectable JSON.
      // Projection exists to drop exactly that bulk before it crosses the wire, so the
      // two can never be equal — and the trimmed payload is what the client
      // legitimately receives. Everything the UI *derives* (title, detail, status,
      // files, …) must still match.
      expect(withoutDetailPayload(deriveWorkLogEntries([projected]))).toEqual(
        withoutDetailPayload(deriveWorkLogEntries([activity])),
      );
