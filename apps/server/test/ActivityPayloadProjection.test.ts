...
describe("projectActivityPayload", () => {
  ...
  it("keeps current web and mobile derived output identical for every tool item type", () => {
    for (const activity of fixtures) {
      ...
      // `detailPayload` is excluded deliberately. This fork's work log retains the
      // raw tool payload verbatim (session-logic sets `entry.detailPayload = payload`)
      // so the entry detail view can render it as inspectable JSON. Projection exists
      // to drop exactly that bulk before it crosses the wire, so the two can never be
      // equal — and the trimmed payload is what the client legitimately receives.
      // Everything the UI *derives* (title, detail, status, files, …) must still match.
