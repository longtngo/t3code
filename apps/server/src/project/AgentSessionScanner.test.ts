...
const makeTempDir = Effect.fn("AgentSessionScanner.test.makeTempDir")(function* (prefix: string) {
  ...
  // Realpathed: macOS resolves TMPDIR through the `/var` -> `/private/var`
  // symlink, and the scanner compares realpaths, so a raw temp path makes a
  // symlink-exclusion assertion read as a miss.
