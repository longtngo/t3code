...
function makeProject(
  ...
  return {
    ...
    title: `Project ${id}`,
    workspaceRoot,
    // Fork-only: the workspace-members projection makes this required.
    members: [],
...
const makeHarness = Effect.fn("makeThreadSettlementHarness")(function* (options: HarnessOptions) {
  ...
  const serverSettings = ServerSettingsService.of({
    ...
    // Fork-only: the raw (unvalidated) view the settings form writes through. Deliberately
  ...
  const dependencies = Layer.mergeAll(
    ...
    Layer.mock(OrchestrationEngineService)({
      ...
      subscribeDomainEventsLossless: PubSub.subscribe(domainEvents).pipe(
        Effect.map((subscription) => Stream.fromSubscription(subscription)),
      ),
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          Effect.provide(
            // FORK: cleanup refuses any root whose realpath differs, and macOS's temp dir
            // sits under the `/var` -> `/private/var` symlink, so hand it a resolved base.
            Layer.unwrap(
              Effect.gen(function* () {
                const fs = yield* FileSystem.FileSystem;
                const baseDir = yield* fs.realPath(
                  yield* fs.makeTempDirectoryScoped({ prefix: "t3-storage-cleanup-" }),
                );
