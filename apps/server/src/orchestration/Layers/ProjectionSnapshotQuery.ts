          ...
          ([
            ...
            Effect.gen(function* () {
              ...
              const threads: ReadonlyArray<OrchestrationThread> = threadRows.map((row) => ({
                ...
                titleRegenerationFailedAt: row.titleRegenerationFailedAt ?? null,
                deletedAt: row.deletedAt,
  ...
  const getThreadCheckpointContext: ProjectionSnapshotQueryShape["getThreadCheckpointContext"] = (
    ...
    Effect.gen(function* () {
      ...
      return Option.some({
        ...
        // `mapCheckpointRow`, not upstream's inline literal: that literal omits
        // `memberStates`, which is the exact drop that left the revert guard inert.
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (threadId) =>
    Effect.gen(function* () {
      ...
      return Option.some({
        ...
        titleRegenerationFailedAt: threadRow.value.titleRegenerationFailedAt ?? null,
  ...
  const getThreadDetailByIdBounded = (
    ...
    Effect.gen(function* () {
      ...
      const thread = {
        ...
        activities,
        // `mapCheckpointRow`, not an inline literal: upstream's literal omits
        // `memberStates`, which is the exact drop that left the revert guard inert.
        checkpoints: checkpointRows.map(mapCheckpointRow),
        session: Option.isSome(sessionRow) ? mapSessionRow(sessionRow.value) : null,
      };
