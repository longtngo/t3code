...
describe("CheckpointReactor", () => {
  ...
  effectIt.effect("captures baseline and large turn summaries before completion receipts", () =>
    Effect.gen(function* () {
      ...
      // Short lines on purpose: this is about a large *summary* (25k additions
      // through numstat), and the fork excludes untracked files at or above
      // MAX_UNTRACKED_CHECKPOINT_FILE_BYTES (10 MiB) from capture, so upstream's
      // 448-byte lines put the file outside the checkpoint entirely.
  ...
  it("appends an error activity when no git workspace can be resolved for the thread", async () => {
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        threadId: ThreadId.make("thread-1"),
        // Upstream's turn-count precondition runs first, so a target above the
        // thread's turn count would fail there and never reach the cwd guard
    ...
    expect((failure?.payload as { detail?: string } | undefined)?.detail).toBe(
      "No git workspace is available for this thread. Reverting needs its project directory to be a git repository.",
    );
    expect(harness.provider.rollbackConversation).not.toHaveBeenCalled();
  });

  // Upstream's own fallback case, kept beside the fork's: it additionally covers the
  // project-cwd fallback (no thread worktree at all) and asserts the provider rollback.
  it.each([false, true])(
  ...
  it("reverts without a live provider session by falling back to the thread workspace", async () => {
