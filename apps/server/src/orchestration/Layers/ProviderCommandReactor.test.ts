...
/**
 * Yield-loop until `predicate` holds.
 *
 * `harness.drain()` on its own is NOT a barrier for work the reactor does in response
 * to a domain event. Since upstream #9152 the reactor subscribes to the event PubSub
 * and enqueues onto its worker from a separate fiber, so a drain issued right after
 * `dispatch` can find an empty queue and return before the event has been enqueued at
 * all. Wait on an observable outcome first; add a drain after it when the assertion
 * also needs the unit's follow-up dispatch to have landed.
 */
...
describe("ProviderCommandReactor", () => {
  ...
  effectIt.effect("recovers a closed provider session on turn start by evicting and retrying", () =>
    Effect.gen(function* () {
  ...
  effectIt.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // FORK: the reactor forwards an interrupt only when a LIVE provider session exists
        // (it settles to `stopped` otherwise, rather than erroring). Register one so these
        // reach the provider-interrupt path they are about. See docs/fork/README.md.
        harness.runtimeSessions.push({
          provider: ProviderDriverKind.make("codex"),
          providerInstanceId: ProviderInstanceId.make("codex"),
          status: "running",
  ...
  effectIt.effect("stops a starting session without a bound turn when interrupt fails", () =>
    Effect.gen(function* () {
      ...
      // FORK: the reactor forwards an interrupt only when a LIVE provider session exists
      // (it settles to `stopped` otherwise, rather than erroring). Register one so this
      // reaches the provider-interrupt path it is about. See docs/fork/README.md.
      harness.runtimeSessions.push({
        provider: ProviderDriverKind.make("codex"),
        providerInstanceId: ProviderInstanceId.make("codex"),
        // `hasLiveSessionForThread` only checks presence, not status.
        status: "running",
        runtimeMode: "approval-required",
        threadId: ThreadId.make("thread-1"),
        cwd: "/tmp/provider-project",
      ...
      yield* Effect.promise(() =>
        waitFor(async () => {
          const entry = (await harness.readModel()).threads.find(
  ...
  effectIt.effect("does not overwrite a session that became ready while an interrupt failed", () =>
    Effect.gen(function* () {
      ...
      // The natural completion is dispatched from inside the interrupt mock, so its
      // call is the barrier for the race this test is about; the drain then covers the
      // recovery handler that must decline to overwrite it.
  ...
  // Every adapter phrases "I don't know that request any more" differently, and
  // all of them must reach the friendly stale notice rather than a raw stack.
  it.each([
    ["opencode", "Unknown pending permission request: approval-request-1"],
    ["cursor/grok/claude", "Unknown pending approval request: approval-request-1"],
    ["codex", "Unknown pending Codex approval request: approval-request-1"],
  ])(
    "surfaces stale provider approval request failures without faking approval resolution (%s)",
    async (_provider, detail) => {
      const harness = await createHarness();
      const now = "2026-01-01T00:00:00.000Z";
  ...
  it("does not resurrect a dead session to tell the agent the user cancelled", async () => {
    const harness = await createHarness();
    const now = "2026-01-01T00:00:00.000Z";
    ...
    await harness.runEffect(
      harness.engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-session-set-for-cancel"),
        threadId: ThreadId.make("thread-1"),
        session: {
          threadId: ThreadId.make("thread-1"),
          status: "stopped",
    ...
    // Deliberately no entry in harness.runtimeSessions: the session died with a
    // restart, which is exactly when a user finds a stale prompt and dismisses it.

    await harness.runEffect(
      harness.engine.dispatch({
        type: "thread.activity.append",
        commandId: CommandId.make("cmd-approval-requested-cancel"),
        threadId: ThreadId.make("thread-1"),
        activity: {
          id: EventId.make("activity-approval-requested-cancel"),
          tone: "approval",
          kind: "approval.requested",
          summary: "Command approval requested",
          payload: { requestId: "approval-request-cancel", requestKind: "command" },
          turnId: null,
          createdAt: now,
        },
        createdAt: now,
    ...
    await harness.runEffect(
      harness.engine.dispatch({
        type: "thread.approval.respond",
        commandId: CommandId.make("cmd-approval-respond-cancel"),
        threadId: ThreadId.make("thread-1"),
        requestId: asApprovalRequestId("approval-request-cancel"),
        decision: "cancel",
        createdAt: now,
      }),
    );
    ...
    // Continuing here would spawn a provider subprocess, resume from the stored
    // cursor, and start a billed turn whose opening line is "my decision was:
    // cancel. Please continue." The user's abort must not become new work.
    expect(harness.sendTurn).not.toHaveBeenCalled();
  });
  ...
  it("reacts to thread.session.stop by stopping provider session and clearing thread session state", async () => {
    ...
    // The settled session is dispatched AFTER `stopSession` resolves, so the mock call
    // is not the barrier for the projection.
    await harness.drain();
    const readModel = await harness.readModel();
    const thread = readModel.threads.find((entry) => entry.id === ThreadId.make("thread-1"));
