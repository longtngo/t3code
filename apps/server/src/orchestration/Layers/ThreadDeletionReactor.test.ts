  ...
  const runCleanup = (extraEventFirst?: OrchestrationEvent) =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        // Upstream #8226 replaced `drain` with a sequence fence. Wait through the
        // highest sequence this fixture emits so the drain still covers every event.
        yield* reactor.drainThrough(
          Math.max(deletedEvent.sequence, extraEventFirst?.sequence ?? 0),
        );
...
describe("ThreadDeletionReactor drain", () => {
  ...
  itEffect.effect("waits for a published deletion the subscriber has not consumed yet", () =>
    Effect.gen(function* () {
      ...
      const engine = {
        ...
        // FORK: upstream observes `ProviderService.stopSession`. This reactor dispatches
        // `thread.session.stop` instead, so the cleanup is counted on the engine.
        dispatch: () =>
          Effect.gen(function* () {
      ...
      const layer = ThreadDeletionReactorLive.pipe(
        ...
        Layer.provide(Layer.succeed(PendingBackgroundTaskRepository, pendingTasks)),
