...
const make = Effect.gen(function* () {
  ...
  const resolveResponseStreamingMode = (projectId: ProjectId) =>
    Effect.map(
      // FORK: getRawSettings, not getSettings: this runs on the streaming hot
  ...
  const processRuntimeEvent = (event: ProviderRuntimeEvent) =>
    Effect.gen(function* () {
      ...
      if (reasoningDelta && reasoningDelta.delta.length > 0 && reasoningTurnId) {
        ...
        if (spillChunk.length > 0) {
          yield* dispatchWithFreshCommandId({
            ...
            commandId: providerCommandId(event, "reasoning-delta-buffer-spill"),
            threadId: thread.id,
          ...
          if (Option.isNone(activeReasoningMessageId)) {
            ...
            if (fallbackText !== undefined && !turnAlreadyStreamedReasoning) {
              ...
              if (existingSnapshot === undefined) {
                yield* dispatchWithFreshCommandId({
                  type: "thread.message.reasoning.delta",
                  commandId: providerCommandId(event, "reasoning-delta-snapshot"),
                  threadId: thread.id,
                ...
                yield* dispatchWithFreshCommandId({
                  ...
                  commandId: providerCommandId(event, "reasoning-complete-snapshot"),
                  threadId: thread.id,
                  messageId: snapshotMessageId,
                  turnId,
                  createdAt: now,
        ...
        if (turnId) {
          ...
          for (const requestId of pendingRequestIds) {
            yield* orchestrationEngine.dispatch({
              ...
              commandId: providerCommandId(event, "terminal-user-input-resolved"),
              threadId: thread.id,
              activity: {
      ...
      if (event.type === "task.completed") {
        // The runtime context above is a three-column read; the wake decision
        // needs the whole shell (approvals, pending input, archive state), and
        // task completions are rare enough to pay for it here.
        const wakeThread = yield* resolveThreadShell(thread.id);
        if (!wakeThread) return;
