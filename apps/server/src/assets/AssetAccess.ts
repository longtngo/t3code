...
const finalizeWorkspaceFileAsset = Effect.fn("AssetAccess.finalizeWorkspaceFileAsset")(
  function* (input: {
    ...
    // FORK: audio and video are admitted here rather than by widening
    // `isWorkspacePreviewEntryPath`: that predicate is browser ∪ image and also gates the
    // "open in a browser preview" affordances, which a video has no use for.
    if (
    ...
    const imageDimensions = HEADER_IMAGE_EXTENSIONS.has(
      ...
      : null;
    return {
      // FORK: audio and video take the exact branch with images, and the reason is the
      // token, not tidiness: the `workspace-file` alternative is a directory-scoped bearer
