...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    ...
    // Layered over the real environment, not used in place of it: the fake CLI's
    // launcher is a `#!/bin/sh` script that runs `exec node`, so a bare override
    // object leaves it without a PATH that can find this machine's node.
