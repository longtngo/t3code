...
import { type ProbeResult, type RegistryEntry, buildSample, planLoad, probeTarget } from "./LlmServeManager.logic.ts";
...
const settings = (models: unknown[], ramBudgetBytes = 0): LocalLlmSettings =>
  ({ ramBudgetBytes, providers: { "mlx-serve": { visible: true, modelsDir: "/models" } }, models }) as never;
...
describe("planLoad", () => {
  ...
  it("rejects when already loaded", () => {
    const reg = new Map<string, RegistryEntry>([
      ["c1", { configId: "c1", providerId: "mlx-serve", modelId: "x", pid: 1, port: 8765, estBytes: 1, state: "loading" }],
    ]);
    expect(planLoad("c1", settings([mlxConfig]), reg, HUGE)).toMatchObject({ ok: false, kind: "already_online" });
  });
  ...
  it("reassigns the port when the config's port is taken", () => {
    const reg = new Map<string, RegistryEntry>([
      ["other", { configId: "other", providerId: "mlx-serve", modelId: "y", pid: 2, port: 8765, estBytes: 1, state: "loading" }],
    ]);
...
describe("buildSample", () => {
  ...
  it("reports loading when registered but not yet reachable", () => {
    const reg = new Map<string, RegistryEntry>([
      ["c1", { configId: "c1", providerId: "mlx-serve", modelId: "x", pid: 9, port: 8765, estBytes: GiB(19), state: "loading" }],
    ]);
  ...
  it("reports online and counts RAM when the probe is reachable", () => {
    ...
    const probes = new Map<string, ProbeResult>([["c1", { reachable: true, model: { id: "x", loaded: true, state: "ready" } }]]);
  ...
  it("marks external-provider rows as not managed", () => {
    const row = buildSample(settings([ds4External]), new Map(), new Map(), HUGE).providers[0]!.models[0]!;
...
describe("probeTarget", () => {
  it("uses the registry port when present, else the config port", () => {
    expect(probeTarget("c1", settings([mlxConfig]), new Map())).toEqual({ host: "127.0.0.1", port: 8765 });
    const reg = new Map<string, RegistryEntry>([
      ["c1", { configId: "c1", providerId: "mlx-serve", modelId: "x", pid: 1, port: 8790, estBytes: 1, state: "loading" }],
    ]);
    expect(probeTarget("c1", settings([mlxConfig]), reg)).toEqual({ host: "127.0.0.1", port: 8790 });
  });
});
