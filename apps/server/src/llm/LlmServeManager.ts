...
export const make = Effect.fn("makeLlmServeManager")(function* () {
  ...
  const list: Effect.Effect<LlmServeListResult> = Effect.gen(function* () {
    ...
    const results = yield* Effect.forEach(
      targets,
      ([id, target]) => probeOne(target.host, target.port).pipe(Effect.map((r) => [id, r] as const)),
      { concurrency: 8 },
    );

    return buildSample(lm, reg, new Map(results), os.totalmem());
  });
  ...
  const load = (configId: string): Effect.Effect<{ pid: number; port: number }, LlmServeError> =>
    loadSemaphore.withPermits(1)(
      Effect.gen(function* () {
        const lm = yield* readSettings;
        const live = yield* Ref.get(registry);
        const plan = planLoad(configId, lm, snapshot(live), os.totalmem());
        if (!plan.ok) {
        ...
        if (launch.executable.includes("/") && !fs.existsSync(launch.executable)) {
        ...
        if (!fs.existsSync(launch.modelPath)) {
          return yield* new LlmServeError({
            kind: "not_found",
