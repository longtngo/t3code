...
> =>
  Effect.gen(function* () {
    ...
    // Bumped by `cancel`. A prompt waiting on the serialization permit has run
    // none of the body below, so it is not in `activePromptRef` and cancel
    ...
    const cancel = Effect.gen(function* () {
      ...
      // still parked on the serialization permit and so invisible to the
      // interrupt below. Bumped before `getStartedState` so a cancel that
      // arrives while the session is still starting is not lost.
      yield* Ref.update(promptCancelGenerationRef, (generation) => generation + 1);
            ...
            .withPermit(
              Effect.gen(function* () {
                ...
                return yield* Effect.acquireUseRelease(
                  ...
                  (activePrompt) =>
                    Effect.gen(function* () {
                      // Between the check above and the registration in `acquire` the
                      // prompt is in neither place `cancel` looks, so a cancel landing
                      // in that window would bump a generation nobody re-reads and find
                      // no fiber to interrupt. Re-check now that we are registered;
                      // `release` below interrupts the fiber and clears the ref.
                      //
                      // Not under `wait-for-prompt`: there `cancel` holds the dispatch
                      // permit for its whole body, so it always bumps AFTER this prompt
                      // registered and is itself awaiting this fiber. Abandoning here
                      // would interrupt the very response that cancel is waiting for,
                      // which is the wait that mode exists for.
                      if (
                        options.cancelBehavior !== "wait-for-prompt" &&
                        (yield* Ref.get(promptCancelGenerationRef)) !== generationAtRequest
                      ) {
                        return cancelledResponse;
                      }
                      return yield* Fiber.join(activePrompt.fiber).pipe(
