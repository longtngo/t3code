...
// `realpathSync` on the OS temp root: macOS resolves TMPDIR through the
// `/var` -> `/private/var` symlink, and the resolver under test realpaths the
// prefix it reports, so a raw temp path never matches.
