...
interface CodexThreadOpenClient {
  ...
  // FORK: widened from upstream's `"thread/start"` to admit `thread/fork`, which opens a
  // thread the same way but from a parent (the fork-thread feature). Resume stays on
  // upstream's `raw` channel below, because only resume needs the undecoded response.
