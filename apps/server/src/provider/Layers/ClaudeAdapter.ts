...
/**
 * Falls back to the CLI's structured failure signals when a result carries no
 ...
 */
function claudeCliContextWindow(
  catalog: ClaudeModelCatalog,
  ...
  switch (modelSelection?.model) {
    ...
    case "claude-opus-4-6":
      return resolveClaudeCatalogContextWindow(catalog, modelSelection) === "1m"
        ? 1_000_000
        : 200_000;
  }
  return undefined;
...
function claudeUnarmedModelContextWindow(
  ...
  return claudeCliContextWindow(catalog, modelSelection);
}
...
function compactBoundaryTokenUsageSnapshot(
  ...
  // FORK: after a compaction the context window shrinks to `post_tokens`. Emit an
  // authoritative snapshot that fully resets the meter to the post-compact size:
  // `lastUsedTokens` defaults to `post_tokens` and no pre-compact
  // `totalProcessedTokens` high-water mark is carried forward. Upstream #9293 does
  // the opposite — it takes the pre-compact total as a third argument and reports
  // `pre_tokens` as the before-figure — which would leave the meter pinned at the
  // old usage and defeat the reset the compaction is meant to reflect.
  return makeClaudeTokenUsageSnapshot({
...
export const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const handleSystemMessage = Effect.fn("handleSystemMessage")(function* (
    ...
    switch (message.subtype) {
      ...
      default:
        ...
        // release adding a subtype fails this typecheck instead of slipping
        // through unnoticed.
  ...
  const startSession: ClaudeAdapterShape["startSession"] = Effect.fn("startSession")(
    function* (input) {
      ...
      const cliContextWindow =
        claudeCliContextWindow(modelCatalog, modelSelection) ?? initialContextWindow;
      const autoCompactWindow =
        ...
        claudeUnarmedModelContextWindow(modelCatalog, modelSelection) ??
  ...
  const startTurnNow = Effect.fn("startTurnNow")(function* (
    ...
    if (modelSelection?.model) {
      ...
      const switchedContextWindow = selectedClaudeContextWindow(modelCatalog, modelSelection);
    ...
    // FORK: upstream records a native turn boundary only when the send is NOT a steer.
    // A mid-turn send queues here (registry 5) and every turn reaches the provider
    // through `startTurnNow`, so each one opens a boundary and carries its turn id as
    // the message uuid — which is what `rollbackThread` looks the boundary up by.
    context.turnStartMessageIds.push(turnId);
    yield* updateResumeCursor(context);
    yield* Queue.offer(context.promptQueue, {
      type: "message",
      message: { ...message, uuid: turnId as NonNullable<SDKUserMessage["uuid"]> },
