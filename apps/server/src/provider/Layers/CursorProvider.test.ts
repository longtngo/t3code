  ...
  it("discovers recursive project skills with project precedence", async () =>
    await runNode(
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        // FORK: realpath both roots. On macOS `os.tmpdir()` is a symlink into
        // `/private/var`, and skill discovery reports resolved paths.
        const userHome = yield* fileSystem.realPath(
  ...
  it("treats a symlinked skill outside the root as a package boundary", async () =>
    await runNode(
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        // Realpathed like the sibling test above: macOS resolves TMPDIR through the
        // `/var` -> `/private/var` symlink, and discovery reports realpaths.
        const userHome = yield* fileSystem.realPath(
          yield* fileSystem.makeTempDirectory({
            directory: NodeOS.tmpdir(),
            prefix: "cursor-skills-home-",
