...
usageLimitLayer("CodexAdapterLive usage limits", (it) => {
  // One more event than upstream takes, and one more in the expected sequence: a rate-limits
  // notification also feeds the fork's usage meter, so it emits `account.usage.updated` beside
  // `account.rate-limits.updated` (see this file's own meter tests).
  ...
  it.effect("names the session window for a plan limit", () =>
    Effect.gen(function* () {
      const { adapter, runtime } = yield* startUsageLimitRuntime();
      // 4, not upstream's 3: the fork's usage meter adds an event (see above).
      const eventsFiber = yield* adapter.streamEvents.pipe(
        Stream.take(4),
        Stream.runCollect,
        Effect.forkChild,
