...
// Real directories: `startSession` fails fast with ProviderWorkspaceMissingError
// when the workspace path is not a directory, so a fabricated cwd no longer
// reaches the adapter.
const makeWorkspaceDir = (name: string) =>
  NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), `t3-provider-service-${name}-`));

const persistedCwd = makeWorkspaceDir("persisted-cwd");
...
function makeFakeCodexAdapter(
  ...
  const withdrawQueuedTurn = vi.fn((): Effect.Effect<boolean, ProviderAdapterError> =>
  ...
  it.effect("writes the thread's subagent flag file before recovering a session", () =>
    Effect.gen(function* () {
      ...
      yield* provider.startSession(threadId, {
        ...
        providerInstanceId: codexInstanceId,
        threadId,
        cwd: makeWorkspaceDir("subagent-flag-recover"),
        runtimeMode: "full-access",
      });
...
describe("switching instances that share a continuation group", () => {
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        yield* directory.upsert({
          ...
          runtimePayload: { cwd: persistedCwd },
        });
  ...
  it.effect("carries the persisted cursor and cwd to an instance with the same key", () =>
    Effect.gen(function* () {
      ...
      assert.equal(startInput.cwd, persistedCwd);
