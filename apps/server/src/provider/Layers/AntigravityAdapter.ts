...
/** Realpath of `dir` if it exists, else of its nearest existing ancestor with the missing tail re-joined. */
const realPathNearestAncestor = Effect.fn("AntigravityAdapter.realPathNearestAncestor")(function* (
...
const resolveClientFilePath = Effect.fn("AntigravityAdapter.resolveClientFilePath")(
  function* (input: {
    ...
    // FORK: realpath the nearest EXISTING ancestor. A write that creates a new
    // directory has no real parent yet; falling back to the unresolved path made
    // every such write under a symlinked root (macOS `/var` -> `/private/var`)
    // read as outside the workspace while the roots below are realpathed.
    const parent = yield* realPathNearestAncestor(input.fileSystem, path, path.dirname(resolved));
...
export const makeAntigravityAdapter = Effect.fn("makeAntigravityAdapter")(function* (
  ...
  return {
    ...
    // Antigravity has no account-usage concept, so a refresh reaches nobody.
    ...
    // Antigravity holds a mid-turn prompt on the ACP semaphore, below any
