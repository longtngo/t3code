...
export function makeOpenCodeAdapter(
  ...
  return Effect.gen(function* () {
    ...
    /**
     * Identity-guarded eviction. OpenCode has no per-thread Semaphore, so a recovery path
     * or a concurrent start can genuinely replace this thread's context while a caller is
     * yielding. Only evict if the map still holds THIS context, or a freshly installed
     * live session is orphaned by the delete. (Upstream #8480 extracted this helper; the
     * guard and the reason for it are the fork's.)
     */
