...
it.layer(NodeServices.layer)("server settings", (it) => {
  ...
  it.effect("never lets a settings patch turn authentication off", () =>
    Effect.gen(function* () {
      ...
      // The flag never reached disk, so the next boot still authenticates.
      assert.isFalse("disableAuthentication" in persisted);
      // Control, or "absent" would also pass if nothing had been written at all:
      // the file is no longer empty. Upstream #8176 deliberately persists explicit
      // `providers.*.enabled` flags so an upgrade cannot silently turn off a
      // provider the user had enabled, and that is the only key that survives here.
      assert.deepEqual(Object.keys(persisted), ["providers"]);
    }).pipe(Effect.provide(makeServerSettingsLayer())),
  );
