...
const setup = Effect.gen(function* () {
  // Realpath it: on macOS `os.tmpdir()` is the `/var` symlink to `/private/var`,
  // and the service reports the RESOLVED home, so an unresolved fixture path can
  // never match `fingerprint.resolvedHomePath` (registry 9).
