...
const STATUS_UPSTREAM_REFRESH_ENV = Object.freeze({
  ...
  // fetch timeout fires, burning a full subprocess on every doomed refresh.
...
export function statusUpstreamRefreshFailureCooldown(
  ...
  const exponent = Math.min(Math.max(0, consecutiveFailures - 1), 30);
  const cooldownMs =
...
export const makeGitVcsDriverCore = Effect.fn("makeGitVcsDriverCore")(function* () {
  ...
  const fetchRemoteForStatus = (
    ...
    return executeGit(
      ...
      // needing auth fails fast instead of blocking until the timeout. Without
      // this, every doomed refresh burns a full subprocess for the whole timeout
      // window, which is what pinned a CPU core rather than the retry rate alone.
      // Tradeoff: an HTTPS remote that genuinely requires credentials will never
      // authenticate here, so its ahead/behind status stops refreshing (it settles
      // into the failure backoff). This is a background best-effort refresh, and
      // hanging is worse.
      [
        "--git-dir",
        gitCommonDir,
  ...
  const recordStatusRemoteRefreshFailure = (cacheKey: StatusRemoteRefreshCacheKey): number => {
  ...
  const prepareReviewIndex = Effect.fn("prepareReviewIndex")(function* (
    ...
    // FORK: a copy is stamped "now", so an entry rewritten in the index's own second
    // stops looking racy and a same-size edit reads as unchanged. Re-stamp the copy
    // below its source, as checkpoint capture does (see copiedIndexStampSeconds).
    const sourceMtime = indexExists ? (yield* fileSystem.stat(indexPath)).mtime : Option.none();
    ...
    // Both commands above rewrite the index, so the stamp goes on last.
    if (Option.isSome(sourceMtime)) {
      const indexTime = GitVcsDriver.copiedIndexStampSeconds(sourceMtime.value.getTime());
      if (indexTime > 0) yield* fileSystem.utimes(tempIndexPath, indexTime, indexTime);
    }
    return env;
  });
