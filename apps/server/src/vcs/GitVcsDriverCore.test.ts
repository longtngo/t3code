...
it.effect("backs off and logs failed fetch attempts across linked worktrees", () =>
  Effect.scoped(
    Effect.gen(function* () {
      ...
      // FORK: the fork logs the transition into failure once per outage, not every
      // failed attempt (upstream's per-failure warning was rejected), so the count
      // stays at 1 while the backoff below still paces the fetches.
      assert.include(warnings[0], "git.status.upstreamRefreshFailing");
