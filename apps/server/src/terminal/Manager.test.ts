...
it.layer(
  ...
  it.effect.each([
    ...
    {
      // FORK: `homePath` sets HOME here and `configDirPath` sets CLAUDE_CONFIG_DIR
      // (upstream maps homePath onto CLAUDE_CONFIG_DIR). Retargeted rather than
      // dropped - the behaviour it describes, config beating the instance
      // environment, holds either way.
