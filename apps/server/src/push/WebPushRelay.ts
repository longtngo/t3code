...
const vapidPersistenceError = (message: string, cause?: unknown) =>
...
const readVapidKeyPair = Effect.fn("readVapidKeyPair")(function* (
  ...
  return yield* decodeVapidKeyPair(new TextDecoder().decode(encoded)).pipe(
    Effect.mapError((cause) => vapidPersistenceError("Failed to decode VAPID key pair.", cause)),
  );
});
...
export const getOrCreateVapidKeys = Effect.fn("getOrCreateVapidKeys")(function* (
  ...
  const encoded = yield* encodeVapidKeyPair(generated).pipe(
    Effect.mapError((cause) => vapidPersistenceError("Failed to encode VAPID key pair.", cause)),
  );
        ...
        ? readVapidKeyPair(secrets).pipe(
            Effect.flatMap((concurrent) =>
              concurrent !== null
                ...
                : Effect.fail(
                    vapidPersistenceError(
                      "Failed to read VAPID key pair after concurrent creation.",
                    ),
                  ),
            ),
          )
