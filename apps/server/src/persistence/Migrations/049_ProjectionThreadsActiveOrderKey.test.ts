  ...
  (it) => {
    it.effect("migrates old threads without changing their timestamps or assigning an order", () =>
      Effect.gen(function* () {
        ...
        // FORK: this migration keeps upstream's 049 filename but is applied as id 58,
        // because ids 49-57 are already spent on live fork databases (registry section 1).
        // The two ids below are applied ids, not filename numbers.
        yield* runMigrations({ toMigrationInclusive: 57 });
        // Control: without this the test passes whether or not the renumbering is right,
        // because a column that was added earlier than intended is indistinguishable here.
        const beforeColumns = yield* sql<{ readonly name: string }>`
        SELECT name FROM pragma_table_info('projection_threads')
      `;
        assert.isFalse(beforeColumns.some((column) => column.name === "active_order_key"));
