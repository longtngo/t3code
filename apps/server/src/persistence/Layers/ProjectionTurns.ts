import { OrchestrationCheckpointFile, OrchestrationCheckpointMemberState } from "@t3tools/contracts";
...
const ProjectionTurnDbRowSchema = ProjectionTurn.mapFields(
  Struct.assign({
    ...
    checkpointMemberStates: Schema.NullOr(
    ...
    execute: (row) =>
      sql`
        ...
        DO UPDATE SET
          ...
          checkpoint_member_states_json = excluded.checkpoint_member_states_json
      `,
  });
    ...
    execute: ({ threadId }) =>
      sql`
        SELECT
          ...
          checkpoint_member_states_json AS "checkpointMemberStates"
        FROM projection_turns
        WHERE thread_id = ${threadId}
        ORDER BY
          CASE
