...
import * as BackgroundTaskRecoveryWatchdog from "../src/provider/Services/BackgroundTaskRecoveryWatchdog.ts";
import * as ProviderTurnStallWatchdog from "../src/provider/Services/ProviderTurnStallWatchdog.ts";
...
const startupDependencies = Layer.mergeAll(
  ...
  Layer.succeed(ProviderService.ProviderService, {
    ...
    withdrawQueuedTurn: () => Effect.die("unused"),
  ...
  Layer.succeed(BackgroundTaskRecoveryWatchdog.BackgroundTaskRecoveryWatchdog, {
    start: () => Effect.void,
  }),
  ...
  () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(result, {
        // FORK: this fork settles a restart-orphaned session to `stopped`, the
        // same clean resting state its stop path produces, not upstream's
        // `error`. See `reconcileInterruptedTurnsOnBoot`.
