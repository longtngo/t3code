  ...
  useEffect(() => {
    ...
    for (const [threadKey, queuedMessages] of Object.entries(queuedMessagesByThreadKey)) {
      ...
      const delivery = confirmThreadOutboxMessageQueued(nextQueuedMessage).then((queued) => {
        ...
        // created target defers, while a busy existing thread still queues.
        if (deliveryAction === "send") {
          ...
          const liveDeliveryAction = resolveThreadOutboxDeliveryAction({
            // Same shape as the first call: the model derives `isCreation` from the
            // message itself, and takes no `threadBusy` -- this fork queues a
            // mid-turn send rather than steering it (invariant 5), so a busy thread
