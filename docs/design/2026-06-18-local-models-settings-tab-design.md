...
- **Write path needs a contract change (design review CRITICAL).** `localModels` is a
  ...
  so a *replacement* entry (not a merge) is required — otherwise removing a `perModel` /
...
`SettingsPanels.tsx`, which was *just* split for lazy-loading). The route lazy-imports it.
