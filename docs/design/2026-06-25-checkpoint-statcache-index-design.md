...
`git status` reads the **real** index (warm stat cache) and *summarizes* wholly
...
untracked half of the work but, as Arm A shows, the *tracked* re-hash alone still
...
2. Decide whether the real index is a *safe* seed (see "skip-worktree" below). It is
...
> skip/assume probe **must** run with the *plain* process env — **not** `commitEnv`,
...
> resolve/probe the *temp* index (copying it onto itself, probing the wrong file).
> They are read-only queries of the *real* index and run before the copy.
...
existing code already computes `gitCommonDir` for the temp-index *location*, but the
...
seed. With that one carve-out handled, the seed only affects *how much work* `git add`
...
entries** — `git add -A -- .` resolves every unmerged path to stage 0 *before*
...
- **Persistent per-repo checkpoint index** (reuse one index file across turns so its
  ...
  first capture and adds no new persistent state. *Rejected as the primary fix; could
  layer on later if first-capture latency ever matters.*
...
- **Racy-clean window.** If a file is modified within the same filesystem-timestamp
  ...
  not a new correctness regression. *Accepted; do not over-engineer.*
- **Concurrent index writes.** If the user runs a git command that rewrites the index
  while capture copies it, git's lockfile+rename means the copy reads a *complete*
  index (old or new), never a torn one. A slightly stale copy only costs a few extra
  re-hashes (add -A reconciles to disk). *Accepted.*
...
1. **De-spam the failure activity.** `CheckpointReactor` appends a
   ...
   actionable detail (untracked byte count + "add to .gitignore"). *Low risk, small.*
2. **Cheap pre-flight bound** for a huge brand-new untracked drop (where even a warm
   ...
   reported bug, which is dominated by *unchanged* content.
