...
1. **The query can die while the context still looks alive (the race).**
   ...
   _"Query closed before response received"_ (`PU.performCleanup`). The adapter
...
2. **`listSessions` reports the dead session as active.**
   `ClaudeAdapter.listSessions` (`:3787`) returns _every_ map entry regardless of
...
resume cursor from the _persisted_ `ProviderSessionDirectory`
...
_not_ by cause-presence, so the cause-less `requireSession` variant is also
...
so `handleStreamExit`'s teardown of the _old_ context can be parked when Change B
recreates a _new_ context under the same `threadId` (via `startSession`'s
...
- **Reuse `continuePendingRequestAsNewTurn` verbatim.** Rejected: that path carries
  ...
  already _has_ the user's message and should re-send _that_, not a synthesized
...
- Recovery re-sends the user's message on a freshly-resumed session. If the dead
  ...
  failure is at _start_, before the message is enqueued), the resumed session
  could theoretically see it twice. Mitigated: the failure surfaces at
  `setModel`/`setPermissionMode`/`Queue.offer` _before_ the message is delivered
...
- Other adapters (Codex/Cursor/Grok/OpenCode) share both the
  `listSessions`-returns-all shape _and_ the identity-blind `sessions.delete`
