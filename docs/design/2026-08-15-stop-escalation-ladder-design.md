...
2. **Make that force-stop *recover* the thread.** Today a manual session stop leaves the thread
...
- **The watchdog is structurally blind to the case the user is force-stopping.** `shouldTrip`
  ...
  turn is *legitimately* blocked rather than wedged. A turn wedged **inside** a tool therefore never
...
existing tick then drives the resume. Nothing about the recovery path is new; only its *entry
point* is.

This deliberately **bypasses `shouldTrip`** — not by weakening the guard, but by never consulting
it. The guards (threshold, open-tool set, pending-input) exist to decide *whether the machine
should suspect a stall*. A human pressing Stop twice has already made that judgement, and the
...
reads nor arms it, so a user who cancels a question and later presses Stop still gets a *first*
...
about: `ComposerPrimaryActions` renders Cancel with `onClick={onInterrupt}` — the *same handler*
...
| Safety | Force-stop is destructive and now also *resumes*. Must stay behind the deliberate second press, never a first press or a timer | **must-fix** → the auto-escalation path (`shouldHardStopAfterGrace`) keeps its pending-input carve-out and must not set the recover marker; only the explicit press does |
...
1. **Correctness** — "second press" needs a definition of *when the ledger resets*. If it never
...
2. **Simplicity** — do we need `shouldHardStopAfterGrace` (the timed auto-escalation) at all, or
   ...
   and walked away — but it is the *conservative* half: it never sets the recover marker, and it
