...
described as a *"dedicated cooperative decline for a pending question — never escalates to a hard
stop."*
...
| Interrupting **settles** a pending AskUserQuestion rather than leaving it stuck | `ClaudeAdapter.ts:4059-4065` registers an `abort` listener running `settleAsAborted`, commented *"turn interrupted while waiting for user input"* | ✅ |
...
`ComposerPrimaryActions → ChatComposer → ChatView` to reach the *same* dispatch would be
...
the condition under which the distinction must come back: *if the composer's Stop ever regains
escalation, Cancel must split onto its own non-escalating path again.*
...
| 2 | Correctness | If the turn is *not* running, does interrupting still settle the question? | Yes — the abort listener is registered when the request is created, not per-turn-state, and `interruptThreadTurn` omits `turnId` when no turn is running. Covered by a test that Cancel renders regardless of `isRunning`. |
...
It was only exposed by adding the **negative** half (`isResponding: false` must *not* be disabled),
