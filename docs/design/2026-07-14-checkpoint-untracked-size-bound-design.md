...
**Symptom:** `VCS process timed out in GitVcsDriver.checkpoints.captureCheckpoint: git -C /Users/longngo/src/uni/routing-TSP-algo add -A -- . after 30000ms` — recurring _after_ the prior fix `d5e276e34`.
...
must hash → **zlib-deflate → write** a new blob for _every untracked, non-ignored file whose
content is new_. In `routing-TSP-algo`, an active research process periodically regenerates
...
Two facts make this a _class_ of bug, not a one-off:
...
1. **The prior stat-cache fix (`d5e276e34`) is structurally incapable of helping.** It seeds the
   ...
   _tracked, unchanged_ files. The heavy files are **untracked (0 index entries)** — no stat
...
-fd` deletes every untracked, non-ignored file not in the checkpoint tree — **but respects
  `.gitignore`** (no `-x`). ✓ **This is the critical constraint:** any file capture _skips_ would
  be _deleted_ on the next restore. A size-bound MUST be applied symmetrically to both ends, or
...
data artifacts like matrices, caches, model weights). _(Env-configurability cut per the Stage-6
...
4. `log(info)` a compact one-liner: how many files / how many bytes were skipped and the
   threshold, so a user can see _why_ their big artifact isn't in the checkpoint.
...
when it meets a _fully-untracked directory_ (no tracked file anywhere in the subtree) it removes
the **whole directory as a unit**, and a file-level `:(exclude,literal)` pathspec for a file
_inside_ it is silently ignored. Since the motivating artifacts live in `.research-team/extreme/
...
the _small_ untracked siblings — so no pathspec formulation is correct.

**Correct mechanism (verified): exclude via git's ignore machinery, `git clean -e <pattern>`.**
Ignore-aware `clean` _descends into_ an untracked directory that contains excluded content and
...
kept. Symmetry holds by _outcome_ (skip ⟺ preserve), achieved with different flags on each side
...
- **Complete + commit the repo `.gitignore` only** — repo-side band-aid, already failed twice
  (whack-a-mole). Does not generalize. _Kept as an immediate mitigation, not the fix._
...
- **Tracked-only capture (`git add -u`)** — restore's `clean -fd` would then delete _all_
...
- **Cumulative-byte budget instead of per-file** — the measured failures are dominated by _large
  individual_ files (446MB, 42MB); a per-file threshold handles them with the simplest possible
...
- **Predicate re-evaluated at restore time.** A file oversized at capture but shrunk below `S`
  ...
  A file oversized-at-capture that _grew from_ a small captured file is handled correctly:
...
3. **Restore still cleans small untracked files** — a small untracked file (both a top-level one
   and a sibling _inside_ the preserved oversized file's dir) is deleted by restore.
...
6. **Tracked file over S is still captured** — the bound is untracked-only; a large _tracked_
