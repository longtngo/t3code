...
- On an **idle** thread, the unbounded snapshot loads the _full_ history into the store (nothing trims it) — so the 99 MB is real **and** full scrollback exists today.

**Decision:** windowed initial load + **bounded backfill up to a ceiling** (not FE == BE). The ceiling _is_ the (raised) client cap, so the live-event trim and the backfill target are the same number and never fight.
