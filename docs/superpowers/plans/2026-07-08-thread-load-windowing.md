...
import { OrchestrationSubscribeThreadInput, OrchestrationHistoryCursor, OrchestrationThreadHistoryPageInput } from "./orchestration.ts";
...
it("subscribe input accepts window params and stays optional", () => {
  ...
  const windowed = Schema.decodeUnknownSync(OrchestrationSubscribeThreadInput)({ threadId: "t_1", windowTurns: 15, maxRows: 2000 });
  expect(windowed.windowTurns).toBe(15);
});

it("history cursor + page input round-trip", () => {
  const cur = { requestedAt: "2026-07-01T00:00:00.000Z", turnId: "turn_9", checkpointTurnCount: 12 };
  const decoded = Schema.decodeUnknownSync(OrchestrationHistoryCursor)(cur);
  expect(decoded.turnId).toBe("turn_9");
  const page = Schema.decodeUnknownSync(OrchestrationThreadHistoryPageInput)({ threadId: "t_1", beforeTurn: cur, maxTurns: 25, maxRows: 3000 });
...
```ts
it("no window options → full thread unchanged", () => Effect.gen(function* () {
...
it("windowed to latest 15 turns caps rows and reports hasMoreHistory", () => Effect.gen(function* () {
  const q = yield* ProjectionSnapshotQuery;
...
it("maxRows caps before windowTurns when a turn is huge", () => Effect.gen(function* () {
...
it("prependThreadHistory adds older rows and keeps sequence order", () => {
  ...
  s = prepend(s, threadId, { activities: [act("a8", 8), act("a9", 9)], messages: [], proposedPlans: [], checkpoints: [] });
